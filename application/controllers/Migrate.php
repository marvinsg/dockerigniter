...
class Migrate extends CI_Controller
{
	...
	public function __construct()
	{
		...
		if (!is_dir($this->_migration_path)) {
			...
			if (!mkdir($this->_migration_path)) {
				...
				show_error(
					'Could not create the directory application/migrations! Try again and if the error persists try creating it manually!'
				);
			}
	...
	{
		$this->_prompt(
			"This function will delete all the files in the migrations folder,"
	...
	public function last()
	{
		...
		if ($rollback_version) {
			...
			show_error(
				"Looks like your migrations table hasn't been configured properly!"
	...
	protected function _migrations_status()
	{
		...
		if ($this->config->item('migration_enabled') !== true) {
			...
			if (!empty($migration_file_content)) {
				$migration_file_content = str_replace(
				...
				if (!write_file($migration_config_file, $migration_file_content)) {
					...
					$error .= PHP_EOL
					          . 'If the error persists simply goto projectfolder/application/config/migration.php'
					          . PHP_EOL;
			...
			show_error(
				"Looks like your migration configurations aren't set properly. Try getting a fresh copy of config/migration.php and retry!"
			);
		}
	}
	...
	protected function _customize_migrations_table()
	{
		...
		$fields = [
			'latest_file_version' => ['type' => 'INT', 'default' => 0]
		];
	...
	protected function _get_file_content($file_name = '')
	{
		...
		$file_content .= '	 */' . PHP_EOL;
		$file_content .= '	protected $_' . 'table_name = "' . trim($this->_migration_file_table) . '";' . PHP_EOL
		                 . PHP_EOL;
		...
		$file_content .= '		$this->load->dbforge();' . PHP_EOL;
		$file_content .= '	}' . PHP_EOL;
		...
		if (strpos($this->_migration_file_name, 'modify') !== false) {
			$file_content .= '		$this->dbforge->add_column($' . 'this->_table_name, $' . 'this->_fields());'
			                 . PHP_EOL;
		} else {
			$file_content .= '		$this->dbforge->add_field(\'id\');' . PHP_EOL;
			$file_content .= '		$this->dbforge->add_field("`created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP");'
			                 . PHP_EOL;
			$file_content .= '		$this->dbforge->add_field("`updated_at` TIMESTAMP on update CURRENT_TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP");'
			                 . PHP_EOL;
		...
		if (strpos($this->_migration_file_name, 'modify') !== false) {
			...
			$file_content .= '			{' . PHP_EOL;
			$file_content .= '				$this->dbforge->drop_column($' . 'this->_table_name,$' . 'key);'
			                 . PHP_EOL;
			$file_content .= '			}' . PHP_EOL;
			$file_content .= '		}' . PHP_EOL;
		} else {
		...
		if (strpos($this->_migration_file_name, 'modify') !== false) {
			$file_content .= PHP_EOL;
			$file_content .= '	/**' . PHP_EOL;
			$file_content .= '	 * Returns an array of the fields to be used within the up and down functions!'
			                 . PHP_EOL;
			$file_content .= '	 *' . PHP_EOL;
